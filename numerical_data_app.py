...
def data_viewer(df):
    ...
    for group, columns in column_groups.items():
        ...
        if len(fig.data) > 0:  # 只有在有数据时才显示图表
            try:
                y_max = max([trace.y.max() for trace in fig.data if len(trace.y) > 0])
                y_min = min([trace.y.min() for trace in fig.data if len(trace.y) > 0])
                ...
                st.warning(f"在处理 {group} 数据时出现问题: {str(e)}")
                fig.update_layout(
                    title=f'{group}数据',
                    xaxis_title='日期时间',
                    yaxis_title='数值',
